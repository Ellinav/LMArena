...
WARNED_UNKNOWN_IDS = set() # 新增：用于存放已警告过的未知ID
    ...
    while True:
        ...
        if browser_ws:
            try:
                ...
                logger.debug(f"Ping发送失败，可能连接已关闭: {e}")
...
async def add_or_update_endpoint(payload: EndpointUpdatePayload):
    """接收来自油猴脚本的单个端点信息，并更新到内存中的MODEL_ENDPOINT_MAP。"""
    global MODEL_ENDPOINT_MAP
    ...
    new_entry = {
        ...
        "mode": payload.mode,
        "battle_target": payload.battle_target
    }
...
async def export_map(request: Request):
    """导出内存中当前的MODEL_ENDPOINT_MAP为JSON。"""
...
async def import_map(request: Request):
    """从JSON请求体导入数据，并覆盖内存中的MODEL_ENDPOINT_MAP。"""
    global MODEL_ENDPOINT_MAP
    api_key = os.environ.get("API_KEY") or CONFIG.get("api_key")
    if api_key:
        auth_header = request.headers.get('Authorization')
    ...
    try:
        ...
        return {"status": "success", "message": f"Map imported successfully with {len(MODEL_ENDPOINT_MAP)} entries."}
    except json.JSONDecodeError:
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    WARNED_UNKNOWN_IDS.clear() # 清空之前的警告记录
    logger.info("✅ 油猴脚本已成功连接 WebSocket。")
    
    # 注意：我们这里不发送 'connected' 消息了，因为新版油猴脚本已不再需要
        ...
        while True:
            ...
            if request_id in response_channels:
                ...
                if request_id not in WARNED_UNKNOWN_IDS:
                    logger.warning(f"⚠️ 收到未知或已关闭请求的响应: {request_id}。这可能是由请求超时或网络重连引起的。将不再为此ID报告后续警告。")
