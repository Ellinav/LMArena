...
async def add_or_update_endpoint(payload: EndpointUpdatePayload):
    ...
    # 【【【 已修复 】】】
    # 将 .pop("model_name") 修改为 .pop("modelName") 以匹配前端发送的键
    ...
    # 这里也可以选择将更新后的地图立即保存到文件
    try:
        with open('model_endpoint_map.json', 'w', encoding='utf-8') as f:
            json.dump(MODEL_ENDPOINT_MAP, f, indent=4, ensure_ascii=False)
            logger.info("model_endpoint_map.json 文件已成功更新。")
    except Exception as e:
        logger.error(f"写入 model_endpoint_map.json 文件时出错: {e}")
