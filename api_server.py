...
class UpdateIdsPayload(BaseModel):
    sessionId: str
    messageId: str
...
@app.post("/v1/update-ids")
async def update_session_ids_in_memory(payload: UpdateIdsPayload):
    """
    从油猴脚本接收会话ID，并将其更新到服务器的内存中。
    """
    global SESSION_STATE
    SESSION_STATE["session_id"] = payload.sessionId
    SESSION_STATE["message_id"] = payload.messageId

    logger.info("✅ 成功通过API更新了内存中的会话ID。")
    logger.info(f"   - 新 Session ID: {SESSION_STATE['session_id']}")
    logger.info(f"   - 新 Message ID: {SESSION_STATE['message_id']}")
    
    return {"status": "success", "message": "IDs updated in server memory."}
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    await websocket.send_text(json.dumps({"status": "connected", "message": "Connection established"}))
...
async def chat_completions(request: Request):
    ...
    message_id = SESSION_STATE.get("message_id")

    if not session_id or not message_id:
        raise HTTPException(
            status_code=400,
            detail="服务器缺少会话ID。请在浏览器中访问LMArena页面并点击一次'Retry'按钮以同步ID。"
        )
